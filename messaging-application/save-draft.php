	...
	if($result->num_rows == 0){ //if the recipient user doesn't exist
		...
		else
		{
			...
			if (isset($_POST['msgid'])) //updates the already-existing draft.
				$sql = "UPDATE empuk_email SET receiver = '" . $rid . "', subject = '" . $subject . "', content = '" . $msg . "', time = '" . $date . "' WHERE emailID = " . $_POST['msgid'];
			else //inserts the message in to the database.
				$sql = "INSERT INTO empuk_email (sender, receiver, subject, content, draft) VALUES ('" . $sid . "','" . $rid . "','" . $subject . "','" . $msg . "', TRUE)";
